...
class Fruit_db
{
    ...
    public static function get_instance($params = false)
    {
        ...
        if (!isset(self::$_instance[$instance_name]) or null === self::$_instance[$instance_name])
        {
            if (!is_array($params))
            {
                $dbuser = Fruit_config::$dbuser;
                $dbpass = Fruit_config::$dbpass;
                $dbname = Fruit_config::$dbname;
                $dbhost = Fruit_config::$dbhost;
                $dbencoding = Fruit_config::$dbencoding;
            }
    ...
    private function __construct($dbuser, $dbpass, $dbname, $dbhost, $dbencoding)
    {
        ...
        if (Fruit_config::$db_time_zone)
            $this->connect->query('SET time_zone = \'' . Fruit_config::$db_time_zone . '\'');
    }
