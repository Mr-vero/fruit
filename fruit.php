...
define('FRUIT_PATH', str_replace('\\', '/', dirname(__file__))); // str_replace - windows trick
require (FRUIT_PATH . '/fruit_config.php'); // configuration
require (FRUIT_PATH . '/fruit_db.php'); // database class
mb_internal_encoding(Fruit_config::$mbencoding); // sets multibyte encoding globaly
date_default_timezone_set(@date_default_timezone_get()); // fruit code not depends on timezone, but this can fix some warnings
...
class Fruit
{
    ...
    protected $load_view = array(
        'list' => 'fruit_list_view.php',
        'create' => 'fruit_detail_view.php',
        'edit' => 'fruit_detail_view.php',
        'view' => 'fruit_detail_view.php');
    ...
    /** constructor, sets basic fruit vars (they can be changed by public pethods) */
    protected function __construct()
    {
        Fruit_config::$scripts_url = self::check_url(Fruit_config::$scripts_url, true);
        Fruit_config::$editor_url = self::check_url(Fruit_config::$editor_url);
        Fruit_config::$editor_init_url = self::check_url(Fruit_config::$editor_init_url);

        $this->limit = Fruit_config::$limit;
        $this->limit_list = Fruit_config::$limit_list;
        $this->column_cut = Fruit_config::$column_cut;
        $this->show_primary_ai_field = Fruit_config::$show_primary_ai_field;
        $this->show_primary_ai_column = Fruit_config::$show_primary_ai_column;

        $this->benchmark = Fruit_config::$benchmark;
        $this->start_minimized = Fruit_config::$start_minimized;
        $this->remove_confirm = Fruit_config::$remove_confirm;
        $this->upload_folder_def = Fruit_config::$upload_folder_def;

        $this->theme = Fruit_config::$theme;
        $this->is_print = Fruit_config::$enable_printout;
        $this->is_title = Fruit_config::$enable_table_title;
        $this->is_csv = Fruit_config::$enable_csv_export;
        $this->is_numbers = Fruit_config::$enable_numbers;
        $this->is_pagination = Fruit_config::$enable_pagination;
        $this->is_search = Fruit_config::$enable_search;
        $this->is_limitlist = Fruit_config::$enable_limitlist;
        $this->is_sortable = Fruit_config::$enable_sorting;

        $this->language = Fruit_config::$language;

        $this->search_pattern = Fruit_config::$search_pattern;

        $this->demo_mode = Fruit_config::$demo_mode;

        $this->default_tab = Fruit_config::$default_tab;
        $this->prefix = Fruit_config::$dbprefix;
        $this->is_rtl = Fruit_config::$is_rtl;

        $this->strip_tags = Fruit_config::$strip_tags;
        $this->safe_output = Fruit_config::$safe_output;

        $this->lists_null_opt = Fruit_config::$lists_null_opt;

        $this->date_format = array('php_d' => Fruit_config::$php_date_format, 'php_t' => Fruit_config::$php_time_format);
    }
    ...
    {
        if (isset($_POST['fruit']['instance']) && isset($_POST['fruit']['key']) && isset($_POST['fruit']['task']))
        {
            self::init_prepare('post');
            $key = $_POST['fruit']['key'] ? $_POST['fruit']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_POST['fruit']['instance'] ? $_POST['fruit']['instance'] : self::error('Instance name cannot be empty');
            $is_get = false;
        }
        elseif (isset($_GET['fruit']['instance']) && isset($_GET['fruit']['key']) && isset($_GET['fruit']['task']) && $_GET['fruit']['task'] ==
            'file')
        {
            self::init_prepare('get');
            $key = $_GET['fruit']['key'] ? $_GET['fruit']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_GET['fruit']['instance'] ? $_GET['fruit']['instance'] : self::error('Instance name cannot be empty');
            $is_get = true;
        }
        else
        {
        ...
        if (isset($_SESSION['lists']['fruit_session'][$inst_name]['key']) && $_SESSION['lists']['fruit_session'][$inst_name]['key'] ==
            $key)
        {
    ...
    protected static function init_prepare($method = false)
    {
        ...
        {
            case 'post':
                $sess_name = (Fruit_config::$dynamic_session && isset($_POST['fruit']['sess_name']) && $_POST['fruit']['sess_name']) ? $_POST['fruit']['sess_name'] :
                    Fruit_config::$sess_name;
                break;
            case 'get':
                $sess_name = (Fruit_config::$dynamic_session && isset($_GET['fruit']['sess_name']) && $_GET['fruit']['sess_name']) ? $_GET['fruit']['sess_name'] :
                    Fruit_config::$sess_name;
                break;
            default:
                $sess_name = Fruit_config::$sess_name;
                break;
        }
        self::session_start($sess_name);
        if (is_callable(Fruit_config::$before_construct))
        {
            call_user_func(Fruit_config::$before_construct);
        }
    }
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (!session_id() && !Fruit_config::$external_session && !Fruit_config::$alt_session)
        {
            if (!headers_sent())
            {
                session_name($sess_name);
                session_cache_expire(Fruit_config::$sess_expire);
                session_set_cookie_params(0, '/');
                session_start();
            }
            else
                self::error('fRUID can not create session, because the output is already sent into browser. 
                Try to define fRUID instance before the output start or use session_start() at the beginning of your script');
        }
        if (Fruit_config::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                setcookie($sess_name, self::$sess_id, time() + Fruit_config::$alt_lifetime * 60, '/');
            }
            else
                self::error('fRUID can not start session, because the output is already sent into browser. 
                Try to define fRUID instance before the output start or use <strong>Fruit::session_start();</strong> at the beginning of your script');
        }
    }
    ...
    public function nested_table($instance_name = '', $field = '', $inner_tbl = '', $tbl_field = '')
    {
        if ($instance_name && $field && $inner_tbl && $tbl_field)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                $instance = Fruit::get_instance($instance_name); // just another fruit object
                ...
                switch ($type)
                {
                    ...
                    case 'point':
                        ...
                        $def_attr = array( // defaults
                            'text' => Fruit_config::$default_text,
                            'search_text' => Fruit_config::$default_search_text,
                            'zoom' => Fruit_config::$default_zoom,
                            'width' => Fruit_config::$default_width,
                            'height' => Fruit_config::$default_height,
                            'search' => Fruit_config::$default_coord,
                            'coords' => Fruit_config::$default_search);
                        $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = array_merge($def_attr, (array )$attr);
                        break;
                    case 'remote_image':
    ...
    protected function _run_task()
    {
        ...
        switch ($this->task)
        {
            ...
            case 'print':
                ...
                $this->_set_field_types('list', Fruit_config::$print_all_fields);
                $this->theme = 'printout';
                return $this->_list();
                break;
            case 'depend':
            ...
            case 'file':
                ...
                break;
            case 'csv':
                $this->_set_field_types('list', Fruit_config::$csv_all_fields);
                return $this->_csv();
                break;
            case 'list':
            default:
    ...
    protected function render_custom_datagrid()
    {
        ...
        $db = Fruit_db::get_instance($this->connection);
    ...
    protected function render_custom_csv()
    {
        ...
        fputcsv($output, $this->columns_names, Fruit_config::$csv_delimiter, Fruit_config::$csv_enclosure);
        ...
        while ($row = $db->result->fetch_assoc()) // low level result process, saves memory
        {
            ...
            fputcsv($output, $out, Fruit_config::$csv_delimiter, Fruit_config::$csv_enclosure);
        }
    }
    ...
    protected function render_output()
    {
        ...
        {
            $contents = '';
            if (!self::$css_loaded && !Fruit_config::$manual_load)
            {
                $contents .= self::load_css();
            }
            ob_start();
            include (FRUIT_PATH . '/' . Fruit_config::$themes_path . '/' . $this->theme . '/fruit_container.php');
            $contents .= ob_get_contents();
            ob_end_clean();
            unset($this->data);
            if (!self::$js_loaded && !Fruit_config::$manual_load)
            {
    ...
    protected function after_render()
    {
        ...
        if (is_callable(Fruit_config::$after_render))
        {
            call_user_func(Fruit_config::$after_render);
        }
    }
    ...
    public function _csv()
    {
        ...
        $output = fopen('php://output', 'w');
        fwrite($output, chr(0xEF) . chr(0xBB) . chr(0xBF)); // bom
        fputcsv($output, $headers, Fruit_config::$csv_delimiter, Fruit_config::$csv_enclosure);
    ...
    {
        if (isset($_POST['fruit'][$field]))
        {
            if (get_magic_quotes_gpc())
            {
                if (is_array($_POST['fruit'][$field]))
                {
                    array_walk_recursive($_POST['fruit'][$field], array($this, 'stripslashes_callback'));
                }
                else
                {
                    $_POST['fruit'][$field] == stripslashes($_POST['fruit'][$field]);
                }
            }
            if (Fruit_config::$auto_xss_filtering)
            {
            ...
            if (($field == 'postdata' or $field == 'unique') && $_POST['fruit'][$field])
            {
                $data_keys = array_keys($_POST['fruit'][$field]);
                foreach ($data_keys as $k => $key)
                {
                    $data_keys[$k] = $xss ? $xss->xss_clean($this->fieldname_decode($key)) : $this->fieldname_decode($key);
                    if ($xss)
                    {
                        $_POST['fruit'][$field][$key] = $xss->xss_clean($_POST['fruit'][$field][$key]);
                    }
                }
                return array_combine($data_keys, $_POST['fruit'][$field]);
            }
            elseif ($filter)
            {
                switch ($filter)
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_POST['fruit'][$field]) : $_POST['fruit'][$field]);
                        break;
                    case 'int':
                        return (int)$_POST['fruit'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_POST['fruit'][$field]) : $_POST['fruit'][$field]);
                        break;
                    case 'base64':
                        return $xss ? $xss->xss_clean($this->fieldname_decode($_POST['fruit'][$field])) : $this->fieldname_decode($_POST['fruit'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_POST['fruit'][$field]) : $_POST['fruit'][$field];
                        break;
                }
            }
            else
    ...
    {
        if (isset($_GET['fruit'][$field]))
        {
            if (get_magic_quotes_gpc())
            {
                if (is_array($_GET['fruit'][$field]))
                {
                    array_walk_recursive($_GET['fruit'][$field], array($this, 'stripslashes_callback'));
                }
                else
                {
                    $_GET['fruit'][$field] == stripslashes($_GET['fruit'][$field]);
                }
            }
            if (Fruit_config::$auto_xss_filtering)
            {
            ...
            if ($filter)
            {
                ...
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_GET['fruit'][$field]) : $_GET['fruit'][$field]);
                        break;
                    case 'int':
                        return (int)$_GET['fruit'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_GET['fruit'][$field]) : $_GET['fruit'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_GET['fruit'][$field]) : $_GET['fruit'][$field];
                        break;
                }
            }
            else
    ...
    protected function _create($postdata = array())
    {
        ...
        if ($this->before_create)
        {
            ...
            {
                $postdata = new Fruit_postdata($this->result_row, $this);
    ...
    protected function _save()
    {
        ...
        if (!$this->primary_val)
        {
            ...
            $pd = new Fruit_postdata($postdata, $this);
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    if (Fruit_config::$email_enable_html)
                        $message = $alert['message'] . '<br /><br />' . "\r\n" . ($alert['link'] ? '<a href="' . $alert['link'] .
                            '" target="_blank">' . $alert['link'] . '</a>' : '');
                    else
                        $message = $alert['message'] . "\r\n\r\n" . ($alert['link'] ? $alert['link'] : '');
                    $this->send_email($send_to, $alert['subject'], $message, $alert['cc'], Fruit_config::$email_enable_html);
                }
            }
            if ($this->mass_alert_create)
            {
                foreach ($this->mass_alert_create as $alert)
                {
                    ...
                    {
                        $this->send_email($row[$alert['email_column']], $alert['subject'], $message, array(), Fruit_config::$email_enable_html);
                    }
                }
            }
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        ...
        else
        {
            ...
            $this->limit = $this->_post('limit', ($this->limit ? $this->limit : Fruit_config::$limit));
        ...
        if (is_array($this->table_info) && count($this->table_info))
        {
            ...
            foreach ($this->table_info as $table => $types)
            {
                foreach ($types as $row)
                {
                    ...
                    if (!$this->field_null[$field_index] && Fruit_config::$not_null_is_required && !isset($this->validation_required[$field_index]))
                    {
    ...
    protected function _define_field_type($row, $field_index)
    {
        ...
        switch ($type)
        {
            ...
            case 'boolean':
                ...
                if ($max_l == 1 && Fruit_config::$make_checkbox)
                {
            ...
            case 'longtext':
                ...
                if (!isset($this->no_editor[$field_index]) && Fruit_config::$auto_editor_insertion)
            ...
            case 'enum':
                ...
                $this->field_type[$field_index] = Fruit_config::$enum_as_radio ? 'radio' : 'select';
            ...
            case 'set':
                ...
                $this->field_type[$field_index] = Fruit_config::$set_as_checkboxes ? 'checkboxes' : 'multiselect';
                $this->field_attr[$field_index]['values'] = $max_l;
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = $row['Default'];
                break;
            case 'point':
                ...
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = Fruit_config::$default_point ? Fruit_config::$default_point : '0,0';
                break;
        }
    }
    ...
    protected function _render_list()
    {
        ...
        if ($this->column === false)
        {
            ...
            elseif (!Fruit_config::$search_all)
            {
                if ($this->search_columns)
                {
        ...
        $mode = 'list';
        $view_file = FRUIT_PATH . '/' . Fruit_config::$themes_path . '/' . $this->theme . '/' . $this->load_view['list'];
    ...
    protected function _render_details($mode)
    {
        ...
        */
        ...
        if ($this->inner_table_instance && ($mode == 'view' or $mode == 'edit')) // restoring nested objects
        {
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    ...
                    if ($mode == 'view' && Fruit_config::$nested_readonly_on_view)
                    {
                    ...
                    $this->nested_rendered[$inst_name] =
                        '<div class="fruit-nested-container fruit-container"><div class="fruit-ajax" id="fruit-ajax-' . base_convert(rand(), 10,
                        36) . '">' . $instance->render('list') . '</div></div>';
                }
            }
        }

        $view_file = FRUIT_PATH . '/' . Fruit_config::$themes_path . '/' . $this->theme . '/' . $this->load_view[$mode];
        $this->check_file($view_file, 'render');
        ob_start();
        include ($view_file);
        ...
        //$this->data .= '<div class="fruit-nested-container fruit-container"><div class="fruit-ajax" id="fruit-ajax-' .
        //    base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        $this->nested_rendered[$inst_name] = '<div class="fruit-nested-container fruit-container"><div class="fruit-ajax" id="fruit-ajax-' .
        base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        }
        }
        }*/
    ...
    protected function _pagination($total, $start, $limit, $numpos = 10, $numlr = 2)
    {
        if ($total > $limit)
        {
            ...
            '"><a href="javascript:;" class="fruit-action" data-start="' . $limit1 . '">' . $i . '</a></li>';
            }
            }*/
    ...
    protected function _cut($string, $field, $wordsafe = true, $dots = true)
    {
        ...
        $string = html_entity_decode($string, ENT_QUOTES, Fruit_config::$mbencoding);
        ...
        $slen = mb_strlen($strip_string, Fruit_config::$mbencoding);
        if ($slen <= $len || (Fruit_config::$print_full_texts && $this->theme == 'printout'))
        {
            return $this->output_string($string, $this->strip_tags, $safe);
        }
        if ($wordsafe)
        {
            $end = $len;
            while ((mb_substr($strip_string, --$len, 1, Fruit_config::$mbencoding) != ' ') && ($len > 0))
            {
            }
            if ($len == 0)
            {
            ...
            return $this->output_string(mb_substr($strip_string, 0, $len, Fruit_config::$mbencoding), false, $safe) . ($dots ?
                '&#133;' : '');
        }
        return $this->output_string(mb_substr($strip_string, 0, $len, Fruit_config::$mbencoding), false, $safe) . ($dots ?
            '&#133;' : '');
    ...
    {
        return mb_convert_case(str_replace('_', ' ', $text), MB_CASE_TITLE, Fruit_config::$mbencoding);
    }
    ...
    public function _export_vars()
    {
        ...
        {
            if (isset($_SESSION['lists']['fruit_session']) && $_SESSION['lists']['fruit_session'])
            {
                foreach ($_SESSION['lists']['fruit_session'] as $s_key => $s_val)
                { // workaround on some servers session duplication
                    $old_time = isset($s_val['time']) ? (int)$s_val['time'] : 0;
                    if ($time > $old_time + Fruit_config::$autoclean_timeout) // autocleaner

                        unset($_SESSION['lists']['fruit_session'][$s_key]);
                }
            }
        }
        ...
        {
            $_SESSION['lists']['fruit_session'][$inst_name][$item] = $this->{$item};
        }
        $_SESSION['lists']['fruit_session'][$inst_name]['before'] = $this->find_prev_task();

        if (Fruit_config::$alt_session)
        {
            $data = $this->encrypt($_SESSION['lists']['fruit_session']);

            if (class_exists('Memcache'))
            {
                $mc = new Memcache();
                $mc->connect(Fruit_config::$mc_host, Fruit_config::$mc_port);
                $res = $mc->set(self::$sess_id, $data, false, Fruit_config::$alt_lifetime * 60);
            }
            elseif (class_exists('Memcached'))
            {
                $mc = new Memcached();
                $mc->connect(Fruit_config::$mc_host, Fruit_config::$mc_port);
                $res = $mc->set(self::$sess_id, $data, Fruit_config::$alt_lifetime * 60);
            }
            else
            {
                self::error('Can\'t use alternative session. Memcache(d) is not available');
            }
            unset($_SESSION['lists']['fruit_session']);
            if (!$res)
            {
    ...
    public function import_vars($key = false)
    {
        if (Fruit_config::$alt_session)
        {
            ...
            $_SESSION['lists']['fruit_session'] = $this->decrypt($data[0], $data[1]);
            unset($data);
            if (!$_SESSION['lists']['fruit_session'])
            {
        ...
        foreach ($this->params2save() as $item)
        {
            $this->{$item} = $_SESSION['lists']['fruit_session'][$inst_name][$item];
        }
    ...
    {
        return '<span class="fruit-none">' . $value . '</span>';
    }
    ...
    {
        if (Fruit_config::$clickable_list_links)
        {
    ...
    protected function create_radio($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'fruit-radio-label');
    ...
    protected function create_checkboxes($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'fruit-checkboxes-label');
    ...
    protected function create_hidden($name, $value = '', $tag = array())
    {
        return $this->single_tag($tag + array(
            ...
            'type' => 'hidden',
            'value' => $value,
            'name' => $name), 'fruit-input', $this->field_attr[$name], true);
    }
    ...
    protected function create_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out .= $this->open_tag('div', 'fruit-upload-container'); // file and delete button
        $out .= $this->open_tag('div', $this->theme_config('grid_button_group'));
        if ($value)
        {
            //$out .= $this->open_tag('span', 'fruit-file-name');
            ...
            $attr = array(
                ...
                'class' => 'fruit-file-name fruit-' . $ext,
                'target' => '_blank');
        ...
        {
            $out .= $this->open_tag('span', 'fruit-nofile ' . $this->theme_config('no_file'));
        ...
        if (!isset($tag['readonly']) && !isset($tag['disabled']))
        {
            ...
            $attr = array(
                ...
                'value' => $value,
                'type' => 'hidden');
            $out .= $this->single_tag('input', 'fruit-input', $attr, true);
            // upload button
    ...
    {
        $out = '';
        $out .= $this->open_tag('span', $this->theme_config('upload_button'), array('class' => 'fruit-add-file'));
    ...
    protected function create_view_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        {
            $out .= $this->open_tag('span', 'fruit-nofile');
            $out .= $this->lang('no_file') . $this->close_tag('span');
        }
        return $out;
    }
    protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'fruit-upload-container'); // image and delete button
        if ($value)
        {
            $binary = isset($this->upload_config[$name]['blob']) ? true : false;
            if ($binary && !$is_upload)
        ...
        {
            $out .= $this->open_tag('div', $this->theme_config('grid_button_group'));
            $out .= $this->open_tag('span', 'fruit-noimage ' . $this->theme_config('no_file'));
    ...
    protected function create_view_image($name, $value = '', $tag = array())
    {
        ...
        {
            $out .= $this->open_tag('span', 'fruit-noimage');
            $out .= $this->lang('no_image') . $this->close_tag('span');
        }
    ...
    protected function create_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = Fruit_config::$default_point ? Fruit_config::$default_point : '0,0';
        }
    ...
    protected function create_view_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = Fruit_config::$default_point;
        }
        if ($value)
        {
            $tag = array(
    ...
    {
        exit('<div class="fruit-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
            <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">fRUID</span>' . $text . '</div>');
    }
    ...
    protected function _upload_file()
    {
        ...
        if (isset($_FILES) && isset($_FILES['fruit-attach']) && !$_FILES['fruit-attach']['error'])
        {
            $file = $_FILES['fruit-attach'];
    ...
    {
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'fruit-upload-container');
        ...
        $attr = array(
            ...
            'alt' => '');
        $out .= $this->single_tag('img', 'fruit-crop', $attr);
    ...
    protected function _compare($val1, $operator, $val2)
    {
        switch ($operator)
        {
            ...
            case '!=':
                return ($val1 != $val2) ? true : false;
            case '^=':
                return (mb_strpos($val1, $val2, 0, Fruit_config::$mbencoding) === 0) ? true : false;
            case '$=':
                return (mb_strpos($val1, $val2, 0, Fruit_config::$mbencoding) == (mb_strlen($val1, Fruit_config::$mbencoding) -
                    mb_strlen($val2, Fruit_config::$mbencoding))) ? true : false;
            case '~=':
                return (mb_strpos($val1, $val2, 0, Fruit_config::$mbencoding) !== false) ? true : false;
            default:
                return false;
        }
    }
    protected function create_modal($field, $content, $image = false)
    {
        ...
        $out .= $this->open_tag('a', 'fruit_modal', $attr);
        if (Fruit_config::$images_in_grid && $image)
        {
            $out .= $content;
        }
        else
        ...
        if (isset($this->field_type[$field]))
        {
            switch ($this->field_type[$field])
            {
                ...
                case 'image':
                    if ($value)
                    {
                        if (Fruit_config::$images_in_grid)
                        {
                            $settings = $this->upload_config[$field];
                            ...
                            $out .= $this->single_tag('img', '', array(
                                ...
                                'style' => 'max-height: ' . Fruit_config::$images_in_grid_height . 'px;'));
                        }
                        else
                        {
                            $out .= $this->open_tag('a', '', array('target' => '_blank', 'href' => isset($this->upload_config[$field]['url']) ? $this->
    ...
    protected function file_link($field, $primary_val, $thumb = false, $crop = false, $filename = false)
    {
        ...
        if ($thumb !== false)
        {
            $params['fruit']['thumb'] = $thumb;
        }
        if ($crop)
        {
            $params['fruit']['crop'] = $crop;
        }
        if (Fruit_config::$dynamic_session)
        {
            $params['fruit']['sess_name'] = session_name();
        }
        return Fruit_config::$scripts_url . '/' . Fruit_config::$ajax_uri . '?' . http_build_query($params);
    }
    ...
    {
        return htmlspecialchars((string )$text, ENT_QUOTES, Fruit_config::$mbencoding);
    }
    ...
    protected function send_email($to, $subject = '(No subject)', $message = '', $cc = array(), $html = true)
    {
        $header = 'MIME-Version: 1.0' . "\r\n" . 'Content-type: text/' . ($html ? 'html' : 'plain') . '; charset=UTF-8' . "\r\n" .
            'From: ' . Fruit_config::$email_from_name . ' <' . Fruit_config::$email_from . ">\r\n";
        if ($cc)
    ...
    protected function _cell_attrib($field, $value, $order, &$row, $is_sum = false, $row_color = false, $row_class = false)
    {
        ...
        if ($field == $order && $this->is_sortable)
            $column_class[] = 'fruit-current';
        if ($is_sum)
            $column_class[] = 'fruit-sum';
        if ($row_color)
        {
    ...
    {
        if (is_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/' . $this->language . '.ini'))
            self::$lang_arr = parse_ini_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/' . $this->language . '.ini');
        elseif (is_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/en.ini'))
            self::$lang_arr = parse_ini_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/en.ini');
        if ($this->set_lang)
        {
    ...
    {
        if (is_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/' . Fruit_config::$language . '.ini'))
            return parse_ini_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/' . Fruit_config::$language . '.ini');
        elseif (is_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/en.ini'))
            self::$lang_arr = parse_ini_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/en.ini');
    }
    protected function _get_theme_config()
    { // loads theme configuration from ini file
        if (is_file(FRUIT_PATH . '/' . Fruit_config::$themes_path . '/' . $this->theme . '/fruit.ini'))
            $this->theme_config = parse_ini_file(FRUIT_PATH . '/' . Fruit_config::$themes_path . '/' . $this->theme . '/fruit.ini');
        else
            self::error('fruit.ini does not exist in your theme folder');
    }
    protected function lang($text = '')
    {
        $langtext = mb_convert_case($text, MB_CASE_LOWER, Fruit_config::$mbencoding);
        return htmlspecialchars((isset(self::$lang_arr[$langtext]) ? self::$lang_arr[$langtext] : $text), ENT_QUOTES,
            Fruit_config::$mbencoding);
    }
    protected function theme_config($text = '')
    {
        $text = mb_convert_case($text, MB_CASE_LOWER, Fruit_config::$mbencoding);
        return htmlspecialchars((isset($this->theme_config[$text]) ? $this->theme_config[$text] : ''), ENT_QUOTES, Fruit_config::
            $mbencoding);
    }
    ...
    public static function load_css()
    {
        ...
        if (self::$css_loaded)
        {
            self::error('Fruit\'s styles already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }

        self::$css_loaded = true;
        if (Fruit_config::$load_bootstrap)
        {
        ...
        if (Fruit_config::$load_jcrop)
            $out .= '<link href="' . Fruit_config::$scripts_url . '/' . Fruit_config::$plugins_uri .
                '/jcrop/jquery.Jcrop.min.css" rel="stylesheet" type="text/css" />';
        $out .= '<link href="' . Fruit_config::$scripts_url . '/' . Fruit_config::$plugins_uri .
            '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' . Fruit_config::$scripts_url .
            '/' . Fruit_config::$themes_uri . '/' . Fruit_config::$theme . '/fruit.css" rel="stylesheet" type="text/css" />';
        return $out;
    }
    public static function load_js()
    {
        ...
        {
            $language = Fruit_config::$language;
        ...
        if (self::$js_loaded)
        {
            self::error('Fruit\'s scripts already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }
        self::$js_loaded = true;
        if (Fruit_config::$load_jquery)
            $out .= '<script src="' . Fruit_config::$scripts_url . '/' . Fruit_config::$plugins_uri . '/jquery.min.js"></script>';
        if (Fruit_config::$jquery_no_conflict)
        {
            $out .= '
        ...
        if (Fruit_config::$editor_url)
            $out .= '<script src="' . Fruit_config::$editor_url . '"></script>';
        if (Fruit_config::$load_googlemap)
        ...
        $config = array(
            ...
            'editor_init_url' => Fruit_config::$editor_init_url,
            'force_editor' => Fruit_config::$force_editor,
            'date_first_day' => Fruit_config::$date_first_day,
            'date_format' => Fruit_config::$date_format,
            'time_format' => Fruit_config::$time_format,
            'lang' => self::$lang_arr,
            'rtl' => Fruit_config::$is_rtl ? 1 : 0);
        $out .= '
            <script type="text/javascript">
            <!--
            
           	var fruit_config = ' . json_encode($config) . ';
                            
            -->
            </script>';
        if ($language != 'en')
        {
            if (is_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/datepicker/jquery.ui.datepicker-' . $language . '.js'))
                $out .= '<script src="' . Fruit_config::$scripts_url . '/' . Fruit_config::$lang_uri .
                    '/datepicker/jquery.ui.datepicker-' . $language . '.js"></script>';
            if (is_file(FRUIT_PATH . '/' . Fruit_config::$lang_path . '/timepicker/jquery-ui-timepicker-' . $language . '.js'))
                $out .= '<script src="' . Fruit_config::$scripts_url . '/' . Fruit_config::$lang_uri .
    ...
    protected function get_limit_list($limit = 20, $buttons = false)
    {
        ...
        if ($buttons)
        {
            $out .= $this->open_tag(array(
                'tag' => 'div',
                'class' => 'btn-group fruit-limit-buttons',
            ...
            foreach ($this->limit_list as $limts)
            {
                ...
                $out .= $this->open_tag(array(
                    ...
                    'data-limit' => $limts), 'fruit-action') . $this->lang($limts) . $this->close_tag(array('tag' => 'button'));
            }
        ...
        else
        {
            $out .= $this->open_tag(array(
                ...
                'name' => 'limit'), 'fruit-actionlist fruit-data');
            foreach ($this->limit_list as $limts)
            {
    ...
    {
        if (!Fruit_config::$performance_mode)
        {
    ...
    {
        if (!is_array($value) && !Fruit_config::$performance_mode && $value)
        {
    ...
    protected function send_http_request($url, $data, $method, $return_result = false)
    {
        ...
        {
            case 'get':
                $ch = curl_init($path . ((mb_strpos($path, '?', 0, Fruit_config::$mbencoding) === false) ? '?' : '&') . $data);
                break;
            case 'post':
                $ch = curl_init($path);
        ...
        if (Fruit_config::$use_browser_info)
        {
        ...
        'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        {
            $tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array('class' => 'fruit-tabs')) . $this->
            ...
            if ($this->nested_rendered && Fruit_config::$nested_in_tab)
            {
    ...
    protected function get_table_tooltip()
    {
        ...
        if ($this->table_tooltip)
        {
            ...
            $out .= $this->open_tag(array(
                'tag' => 'a',
                'href' => 'javascript:;',
                'class' => 'fruit-tooltip fruit-button-link',
    ...
    protected function render_search()
    {
        ...
        if ($this->is_search)
        {
            ...
            $attr = array('class' => 'fruit-search-toggle', 'href' => 'javascript:;');
            if ($this->search or Fruit_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            $out .= $this->open_tag('a', $this->theme_config('search_open'), $attr);
            $out .= $this->lang('search') . $this->close_tag('a');
            $attr = array('class' => 'fruit-search');
            if (!$this->search && !Fruit_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            $out .= $this->open_tag('span', $this->theme_config('search_container'), $attr);
            if (Fruit_config::$search_all)
            {
            ...
            $attr = array('class' => 'fruit-data', 'name' => 'column');
            if ($this->search_default === false && !$this->search_columns)
            {
                $out .= $this->open_tag(array('tag' => 'input', 'type' => 'hidden'), 'fruit-columns-select ' . $this->theme_config('search_fieldlist'),
                    $attr);
            }
            else
            {
                $out .= $this->open_tag('select', 'fruit-columns-select ' . $this->theme_config('search_fieldlist'), $attr);
    ...
    {
        $attr_preset = array('class' => 'fruit-searchdata', 'name' => 'phrase');
        if ($field == $this->column)
        {
            $class = 'fruit-search-active';
        }
        else
        {
            $class = '';
        ...
        switch ($attr['data-type'])
        {
            ...
            case 'timestamp':
                if (!isset($fieldlist['date']) or $field == $this->column)
                {
                    ...
                    $attr_range = array(
                        'class' => 'fruit-daterange fruit-searchdata ' . $this->theme_config('search_range'),
                        'name' => 'range',
                        'data-fieldtype' => 'date');
                    if ($field != $this->column)
                    {
                    ...
                    $fieldlist['date'] .= $this->open_tag('option', '', array('value' => '')) . $this->lang('choose_range') . $this->
                        close_tag('option');
                    if (Fruit_config::$available_date_ranges)
                    {
                        foreach (Fruit_config::$available_date_ranges as $range)
                        {
                    ...
                    $fieldlist['date'] .= $this->single_tag('input', 'fruit-datepicker-from ' . $class . ' ' . $this->theme_config('search_from'),
                        $attr);
                    $attr['name'] = 'phrase][to';
                    $attr['value'] = (isset($phrase['to']) && $field == $this->column) ? $phrase['to'] : '';
                    $fieldlist['date'] .= $this->single_tag('input', 'fruit-datepicker-to ' . $class . ' ' . $this->theme_config('search_to'),
                        $attr);
                }
                break;
            case 'select':
    ...
    {
        $out = '';
        $out .= $this->open_tag($row, 'fruit-th');
        if ($this->is_numbers)
        {
            $out .= $this->open_tag($item, 'fruit-num') . '&#35;' . $this->close_tag($item);
        }
        ...
        {
            $out .= $this->open_tag($item, 'fruit-actions') . '&nbsp;' . $this->close_tag($item);
        }
        foreach ($this->columns as $field => $fitem)
        {
            if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' or $this->field_type[$field] ==
            ...
            if ($this->is_sortable)
            {
                ...
                {
                    $class .= ' fruit-current fruit-' . $this->order_direct;
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
                ...
                $out .= $this->open_tag($row_tag, 'fruit-row fruit-row-' . $i);
                if ($this->is_numbers)
                {
                    $out .= $this->open_tag($item, 'fruit-num', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class)) . ($key +
                        $this->start + 1) . $this->close_tag($item);
                }
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'left')
                {
                    $out .= $this->open_tag($item, 'fruit-actions', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
                ...
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right')
                {
                    $out .= $this->open_tag($item, 'fruit-actions' . (Fruit_config::$fixed_action_buttons ? ' fruit-fix' : ''), $this->
        ...
        else
        {
            ...
            $out .= $this->open_tag($row_tag, 'fruit-row') . $this->open_tag($item, '', array('colspan' => $j)) . $this->lang('table_empty') .
    ...
    protected function render_grid_footer($row = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        {
            $out .= $this->open_tag($row, 'fruit-tf');
            if ($this->is_numbers)
            {
                $out .= $this->open_tag($item, 'fruit-num') . '&Sigma;' . $this->close_tag($item);
            }
    ...
    protected function render_benchmark($tag = array('tag' => 'span'))
    {
        if ($this->benchmark)
        {
            return $this->open_tag($tag, 'fruit-benchmark') . $this->benchmark_end() . $this->close_tag($tag);
        }
        return '';
    }
    protected function render_control_fields()
    {
        ...
        $out .= $this->single_tag($tag, '', array('name' => 'limit', 'value' => ($this->limit ? $this->limit : Fruit_config::$limit)));
    ...
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
    {
        ...
        if (!isset($this->hide_button[$name]))
        {
            ...
            $out .= $this->open_tag($tag, 'fruit-action');
            if ($icon && !$this->is_rtl)
            {
    ...
    protected function csv_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('export_csv', 'csv', '', $class . ' fruit-in-new-window', $icon);
        }
    }
    protected function print_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('print', 'print', '', $class . ' fruit-in-new-window', $icon);
        }
    }
    ...
    protected function get_image_folder($field)
    {
        ...
        {
            $path = $this->check_folder(Fruit_config::$upload_folder_def, 'get_image_folder');
        }
    ...
    {
        $range = array();
        $time = time() /* + 3600 * Fruit_config::$local_time_correction*/;
        $week_day = date('w', $time) /* + Fruit_config::$date_first_day*/;
        switch ($name)
        {
            default:
            case 'today':
            ...
            case 'this_week_today':
                if ($week_day >= Fruit_config::$date_first_day)
                {
                    $offset1 = $week_day - Fruit_config::$date_first_day;
                }
                else
                {
                    $offset1 = 7 - (Fruit_config::$date_first_day - $week_day);
                }
            ...
            case 'this_week_full':
                ...
                $offset2 = 6 - $week_day + Fruit_config::$date_first_day;
                $range['from'] = gmmktime(0, 0, 0, date('n', $time), date('j', $time) - $offset1, date('Y', $time));
    ...
    {
        if ($time)
            return $utc ? gmdate($this->date_format['php_d'] . ' ' . $this->date_format['php_t'], $time) : date(Fruit_config::$php_date_format .
    ...
    protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
    {
        ...
        if ($this->is_title)
        {
            ...
            if (Fruit_config::$can_minimize)
            {
                if ($to_show)
                    $out .= '<span class="fruit-toggle-show fruit-toggle-down"><i class="' . $this->theme_config('slide_down_icon') .
                        '"></i></span>';
                else
                    $out .= '<span class="fruit-toggle-show fruit-toggle-up"><i class="' . $this->theme_config('slide_up_icon') .
                        '"></i></span>';
                $out .= $this->close_tag($tag);
            }
        }
    ...
    {
        if (!Fruit_config::$alt_encription_key)
        {
        ...
        $key = substr(Fruit_config::$alt_encription_key, 0, $ks);
    ...
    {
        $_SESSION['lists']['fruit_session'] = $data;
    }
    public static function export_session()
    {
        return $_SESSION['lists']['fruit_session'];
    }
...
class Fruit_postdata
{
    private $fruit = null;
    private $postdata = array();
    public function __construct($postdata, $fruit)
    {
        $this->fruit = $fruit;
    ...
    {
        $fdata = $this->fruit->_parse_field_names($name, 'Fruit_postdata');
        foreach ($fdata as $key => $fitem)
        {
            $this->postdata[$key] = $value;
        }
        $this->fruit->unlock_field($name);
        return $this;
    }
    ...
    public function get($name)
    {
        ...
        return /*new Fruit_postdata_item*/ ($value);
    }
